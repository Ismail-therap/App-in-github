...
connect_db=function(jdbc,connection.string,uid,pwd){
  jdbcDriver <- JDBC(driverClass="oracle.jdbc.OracleDriver", classPath=jdbc)
  jdbcConnection <- dbConnect(jdbcDriver, connection.string, uid, pwd)
  return(jdbcConnection)
}

RJdbc_con=connect_db("D://driver/ojdbc6.jar", "jdbc:oracle:thin:@//192.168.49.112:1521/BIDB.BIDB01BD.THERAPBD.NET", "bidb", "bidb")
...
eastern_data_smail<-data_from_database(connection=RJdbc_con,quary=quary_es_smail)
...
local_data_smail<-data_from_database(connection=RJdbc_con,quary=quary_loc_smail)
...
filenames_eastern<-list(eastern_data_isp,eastern_data_mar,eastern_data_att,eastern_data_ger,eastern_data_bill,eastern_data_smail)
Combined_eastern<-combining_module(filenames_eastern)


filenames_local<-list(local_data_isp,local_data_mar,local_data_att,local_data_ger,local_data_bill,local_data_smail)
